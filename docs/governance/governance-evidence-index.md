...
| Security checks produce verifiable evidence, not only workflow logs | `.github/workflows/release-trivy.yml` → `trivy-release-scan`, `attest-trivy-results`; `.github/workflows/ci-release-gate.yml` → `dast-analysis`; `.github/workflows/ci-security-deep.yml` → `security-governance`; `.github/workflows/snyk-report.yml` → `snyk-report` | `docs/security-debt.md`; release thresholds in `.github/workflows/release-trivy.yml`; `scripts/check-security-debt.sh` | `trivy-results-backend`; `trivy-results-worker`; `trivy-results-frontend`; `zap-results`; `sbom-*`; `docs/snyk/index.md`; `docs/snyk/html/`; SARIF uploads from `ci-security-deep.yml` | Project Maintainers | Quarterly |
| Container images are signed, attested, and validated through policy before deployment | `.github/workflows/ci-release-gate.yml` → `build-push-backend`, `build-push-frontend`, `build-push-worker`; `.github/workflows/release-build-push-dual-registry.yml` → `build-publish`, `sign-and-attest`, `generate-slsa-provenance-ghcr`, `generate-slsa-provenance-docker`; `.github/workflows/release-trivy.yml` → `trivy-release-scan`, `attest-trivy-results`; `.github/workflows/attest-dast.yml` → `attest-dast-results`; `.github/workflows/gitops-enforce.yml` → `images-verification`, `gitops` | `k8s/policies/cluster/verify-signature.yaml`; `k8s/policies/cluster/verify-trivy.yaml`; `k8s/policies/cluster/verify-zap.yaml`; `k8s/policies/cluster/verify-sbom.yaml`; `k8s/policies/cluster/verify-slsa.yaml` | Release artifacts `digest-backend`, `digest-worker`, `digest-frontend`, `sbom-*`, `trivy-results-*`, `zap-results`; GitOps artifact `kyverno-gitops-log`; `docs/threat-model.md` | Project Maintainers | Quarterly |
